  ...
  return (
    <>
        <nav className='flex items-center py-8 px-16 justify-between dark:bg-darkBlue dark:text-white font-nunito md:px-16 bg-white text-black shadow-lg '>
            <h3 className='text-4xl font-black md:3xl'>Where in the world</h3>
            <button className="theme flex items-center gap-2  cursor-pointer text-2xl " onClick={changeTheme}>
              {theme === 'dark' ? <MdIcons.MdOutlineWbSunny /> : <MdIcons.MdDarkMode/> }
                
                <p className='mode' >{theme === 'dark' ? 'Light' : 'Dark'} mode</p>
            </button>
        </nav>
    </>
  )
